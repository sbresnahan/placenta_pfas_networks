...
# Load transcript-to-gene mapping table
...
create_volcano_plot <- function(deseq2_results, pfas_var, output_dir = ".", 
  ...
  if(any(man.labs==F)){
    ...
    if(any(res_df$ensembl %in% man.labs)){
      res_df$Label <- ifelse(res_df$ensembl %in% man.labs, res_df$Gene, '')
    }else{
      res_df$Label <- ''
    }
  }
  ...
  volcano_plot <- ggplot(aes(x = log2FoldChange,
    ...
    ggrepel::geom_label_repel(
      aes(label = Label),
      size = 3,
      max.overlaps = Inf,       # let ggrepel keep trying instead of dropping labels
      force = 1,                # reasonable repulsion strength
      box.padding = 0.4,        # space around labels
      point.padding = 0.2,      # space between point and label
      segment.color = "grey50", # optional: connector segment
      segment.size = 0.3
    ) +
    theme_minimal() +
...
csv_out = function(result,pfas_var,output_dir){
  
  rrr = result$results
  rrr$transcript_id = rrr$transcript_id
...
vis_efcor <- function(result_mat,result_child,exposure,output_dir){
  ...
  df_merged <- dplyr::inner_join(
    df_mat %>% dplyr::select(transcript_id, gene_id, log2FoldChange, pvalue) %>% dplyr::rename(lfc_maternal = log2FoldChange,
                                                                  pvalue_maternal = pvalue),
    df_child %>% dplyr::select(transcript_id, log2FoldChange, pvalue) %>% dplyr::rename(lfc_fetal = log2FoldChange,
                                                                    pvalue_fetal = pvalue),
    by = "transcript_id"
  )
...
## Maternal
result2 <- run_ruvr_deseq2(se = se_gusto.tx, 
                          pfas_var = all_pfas[1], 
                          covariates = c("sex", "GA"), 
                          k = 8, output_dir = "results", 
                          just_hist = FALSE,fdr_threshold = .1,
                          transcript_to_gene = tx2g.assembly)
volcano <- create_volcano_plot(result, pfas_var = all_pfas[1], output_dir = "results")
csv_out(result,pfas_var = all_pfas[1],output_dir = 'results')
...
result <- run_ruvr_deseq2(se = se_gusto.tx, 
                          pfas_var = all_pfas[2], 
                          covariates = c("sex", "GA"), 
                          k = 0, output_dir = "results", 
                          just_hist = FALSE,fdr_threshold = .1,
                          transcript_to_gene = tx2g.assembly)
volcano <- create_volcano_plot(result, pfas_var = all_pfas[2], output_dir = "results",top_genes=5)
volcano2 <- create_volcano_plot(result, pfas_var = all_pfas[2], output_dir = "results_imprint",
                               man.labs=c("ENSG00000026025","ENSG00000026025","ENSG00000164889"))
csv_out(result, pfas_var = all_pfas[2],output_dir = 'results')
result_PFOA_mat <- result

result <- run_ruvr_deseq2(se = se_gusto.tx, 
                          pfas_var = all_pfas[3], 
                          covariates = c("sex", "GA"), 
                          k = 2, output_dir = "results", 
                          just_hist = FALSE,fdr_threshold = .1,
                          transcript_to_gene = tx2g.assembly)
volcano <- create_volcano_plot(result, pfas_var = all_pfas[3], output_dir = "results")
csv_out(result,pfas_var = all_pfas[3],output_dir = 'results')
...
result <- run_ruvr_deseq2(se = se_gusto.tx, 
                          pfas_var = all_pfas[4], 
                          covariates = c("sex", "GA"), 
                          k = 1, output_dir = "results", 
                          just_hist = FALSE,fdr_threshold = .1,
                          transcript_to_gene = tx2g.assembly)
volcano <- create_volcano_plot(result, pfas_var = all_pfas[4], output_dir = "results")
csv_out(result,pfas_var = all_pfas[4],output_dir = 'results')
...
result <- run_ruvr_deseq2(se = se_gusto.tx, 
                          ...
                          just_hist = FALSE,fdr_threshold = .1,
                          transcript_to_gene = tx2g.assembly)
volcano <- create_volcano_plot(result, pfas_var = all_pfas[5], output_dir = "results")
csv_out(result,pfas_var = all_pfas[5],output_dir = 'results')
...
result <- run_ruvr_deseq2(se = se_gusto.tx, 
                          ...
                          just_hist = FALSE,fdr_threshold = .1,
                          transcript_to_gene = tx2g.assembly)
volcano <- create_volcano_plot(result, pfas_var = all_pfas[6], output_dir = "results")
csv_out(result,pfas_var = all_pfas[6],output_dir = 'results')
...
result <- run_ruvr_deseq2(se = se_gusto.tx, 
                          ...
                          just_hist = FALSE,fdr_threshold = .1,
                          transcript_to_gene = tx2g.assembly)
volcano <- create_volcano_plot(result, pfas_var = all_pfas[7], output_dir = "results")
csv_out(result,pfas_var = all_pfas[7],output_dir = 'results')
...
result <- run_ruvr_deseq2(se = se_gusto.tx, 
                          ...
                          just_hist = FALSE,fdr_threshold = .1,
                          transcript_to_gene = tx2g.assembly)
volcano <- create_volcano_plot(result, pfas_var = all_pfas[8], output_dir = "results")
csv_out(result,pfas_var = all_pfas[8],output_dir = 'results')
...
result <- run_ruvr_deseq2(se = se_gusto.tx, 
                          pfas_var = all_pfas[9], 
                          covariates = c("sex", "GA"), 
                          k = 3, output_dir = "results", 
                          just_hist = FALSE,fdr_threshold = .1,
                          transcript_to_gene = tx2g.assembly)
volcano <- create_volcano_plot(result, pfas_var = all_pfas[9], output_dir = "results")
csv_out(result,pfas_var = all_pfas[9],output_dir = 'results')

result_PFBA_child <- result
vis_efcor(result_PFBA_mat,result_PFBA_child,all_pfas[9],"results")


result <- run_ruvr_deseq2(se = se_gusto.tx, 
                          pfas_var = all_pfas[10], 
                          covariates = c("sex", "GA"), 
                          k = 10, output_dir = "results", 
                          just_hist = FALSE,fdr_threshold = .1,
                          transcript_to_gene = tx2g.assembly)
volcano <- create_volcano_plot(result, pfas_var = all_pfas[10], output_dir = "results")
csv_out(result,pfas_var = all_pfas[10],output_dir = 'results')

result_PFOA_child <- result
vis_efcor(result_PFOA_mat,result_PFOA_child,all_pfas[10],"results")


result <- run_ruvr_deseq2(se = se_gusto.tx, 
                          pfas_var = all_pfas[11], 
                          covariates = c("sex", "GA"), 
                          k = 5, output_dir = "results", 
                          just_hist = FALSE,fdr_threshold = .1,
                          transcript_to_gene = tx2g.assembly)
volcano <- create_volcano_plot(result, pfas_var = all_pfas[11], output_dir = "results")
csv_out(result,pfas_var = all_pfas[11],output_dir = 'results')

result_PFNA_child <- result
vis_efcor(result_PFNA_mat,result_PFNA_child,all_pfas[11],"results")
...
result <- run_ruvr_deseq2(se = se_gusto.tx, 
                          ...
                          just_hist = FALSE,fdr_threshold = .1,
                          transcript_to_gene = tx2g.assembly)
volcano <- create_volcano_plot(result, pfas_var = all_pfas[12], output_dir = "results")
csv_out(result,pfas_var = all_pfas[12],output_dir = 'results')

result_PFDA_child <- result
vis_efcor(result_PFDA_mat,result_PFDA_child,all_pfas[12],"results")
...
result <- run_ruvr_deseq2(se = se_gusto.tx, 
                          ...
                          just_hist = FALSE,fdr_threshold = .1,
                          transcript_to_gene = tx2g.assembly)
volcano <- create_volcano_plot(result, pfas_var = all_pfas[13], output_dir = "results")
csv_out(result,pfas_var = all_pfas[13],output_dir = 'results')

result_PFUnDA_child <- result
vis_efcor(result_PFUnDA_mat,result_PFUnDA_child,all_pfas[13],"results")
...
result <- run_ruvr_deseq2(se = se_gusto.tx, 
                          ...
                          just_hist = FALSE,fdr_threshold = .1,
                          transcript_to_gene = tx2g.assembly)
volcano <- create_volcano_plot(result, pfas_var = all_pfas[14], output_dir = "results")
csv_out(result,pfas_var = all_pfas[14],output_dir = 'results')

result_PFBS_child <- result
vis_efcor(result_PFBS_mat,result_PFBS_child,all_pfas[14],"results")
...
result <- run_ruvr_deseq2(se = se_gusto.tx, 
                          ...
                          just_hist = FALSE,fdr_threshold = .1,
                          transcript_to_gene = tx2g.assembly)
volcano <- create_volcano_plot(result, pfas_var = all_pfas[15], output_dir = "results")
csv_out(result,pfas_var = all_pfas[15],output_dir = 'results')

result_PFHxS_child <- result
vis_efcor(result_PFHxS_mat,result_PFHxS_child,all_pfas[15],"results")


result <- run_ruvr_deseq2(se = se_gusto.tx, 
                          pfas_var = all_pfas[16], 
                          covariates = c("sex", "GA"), 
                          k = 16, output_dir = "results", 
                          just_hist = FALSE,fdr_threshold = .1,
                          transcript_to_gene = tx2g.assembly)
volcano <- create_volcano_plot(result, pfas_var = all_pfas[16], output_dir = "results")
csv_out(result,pfas_var = all_pfas[16],output_dir = 'results')

result_PFOS_child <- result
vis_efcor(result_PFOS_mat,result_PFOS_child,all_pfas[16],"results")


save.image("GUSTO_assembly_DTE.RData")


##### Trans-placental transfer ratios
...
load("GUSTO_assembly_DTE.RData")

# Use SVA to regress out variation due to covariates and surrogate variables
covs_PFAS <- merged_coldata_tx[!is.na(merged_coldata_tx$PFBS_mat) | !is.na(merged_coldata_tx$PFBS_child), ]

expr <- se_gusto.tx@assays@data@listData[["counts"]]
expr <- data.frame(expr)
expr <- as.matrix(expr[,names(expr)%in%covs_PFAS$SampleID])


# --------------------------
# Filter genes/samples
# --------------------------
clean_expression_for_sva <- function(expr_matrix) {
  gene_detection_rate <- rowSums(expr_matrix > 0) / ncol(expr_matrix)
  expr_filtered <- expr_matrix[gene_detection_rate >= 0.2, ]
  
  sample_detection_rate <- colSums(expr_filtered > 0) / nrow(expr_filtered)
  expr_filtered <- expr_filtered[, sample_detection_rate >= 0.1]
  
  gene_vars <- apply(expr_filtered, 1, var)
  expr_filtered <- expr_filtered[gene_vars > quantile(gene_vars, 0.1), ] 
  
  expr_filtered[is.na(expr_filtered)] <- 0
  expr_filtered[is.infinite(expr_filtered)] <- 0
  
  return(expr_filtered)
}

expr_clean <- clean_expression_for_sva(expr)

# --------------------------
# VST transformation
# --------------------------
dds <- DESeqDataSetFromMatrix(countData = round(expr_clean),
                              colData   = covs_PFAS,
                              design    = ~ 1)
expr_PFAS <- assay(varianceStabilizingTransformation(dds))

write.csv(expr_PFAS,"expr_PFAS_WGCNA.csv")
...
# --------------------------
pfas_vars <- c("PFBA_mat", "PFBS_mat", "PFHxS_mat", "PFOS_mat",
               "PFOA_mat", "PFNA_mat", "PFDA_mat", "PFUnDA_mat",
               "PFBA_child", "PFBS_child", "PFHxS_child", "PFOS_child",
               "PFOA_child", "PFNA_child", "PFDA_child", "PFUnDA_child")

pfas_vars <- pfas_vars[pfas_vars %in% colnames(covs_PFAS)]

# Check for missing data
cat("Missing PFAS data per variable:\n")
print(colSums(is.na(covs_PFAS[, pfas_vars])))

# Identify complete cases
complete_cases <- complete.cases(covs_PFAS[, pfas_vars])
cat("\nTotal samples:", nrow(covs_PFAS), "\n")
cat("Complete cases:", sum(complete_cases), "\n")

# Subset to complete cases
covs_PFAS_complete <- covs_PFAS[complete_cases, ]
expr_PFAS_complete <- expr_PFAS[, complete_cases]

# Create design matrices using complete data
pfas_formula <- as.formula(paste("~", paste(pfas_vars, collapse = " + ")))
mod1 <- model.matrix(pfas_formula, data = covs_PFAS_complete)
mod0 <- model.matrix(~ 1, data = covs_PFAS_complete)

# Verify dimensions match
cat("\nexpr_PFAS_complete dimensions:", dim(expr_PFAS_complete), "\n")
cat("mod1 dimensions:", dim(mod1), "\n")
cat("mod0 dimensions:", dim(mod0), "\n")

# --------------------------
# Run SVA
# --------------------------
sva_result <- sva(dat = expr_PFAS_complete, mod = mod1, mod0 = mod0, n.sv = 1)

# --------------------------
# Regress out SV
# --------------------------
expr_corrected <- removeBatchEffect(expr_PFAS_complete,
                                    covariates = sva_result$sv,
                                    design = mod1)



# --------------------------
# Final matrix for WGCNA
# --------------------------
expr_PFAS <- expr_corrected



# Find soft threshold power
powers = c(c(1:10))
sft = pickSoftThreshold(t(expr_PFAS),
                        powerVector = powers, 
                        verbose = 5,
                        networkType = "signed")

png("WGCA_thresholds.png",width=7,height=5,units="in",res=300)
par(mfrow = c(1,2))
cex1 = 0.5
...
# Select 3 for SFT.R.sq > 0.8 and mean.k ~500

save(file="input_for_WGCNA.RData",list=c("expr_PFAS","covs_PFAS"))

# Continue on seadragon

# Load output from WGCNA analysis on seadragon

load("input_for_WGCNA.RData")
load("WGCNA_output.RData")

# Save output for Sierra

# Create new list with cleaned names and results data frames
pfas_DTE_results <- list(
  PFBA_child = result_PFBA_child[["results"]],
  PFBA_mat = result_PFBA_mat[["results"]],
  PFBS_child = result_PFBS_child[["results"]],
  PFBS_mat = result_PFBS_mat[["results"]],
  PFDA_child = result_PFDA_child[["results"]],
  PFDA_mat = result_PFDA_mat[["results"]],
  PFHxS_child = result_PFHxS_child[["results"]],
  PFHxS_mat = result_PFHxS_mat[["results"]],
  PFNA_child = result_PFNA_child[["results"]],
  PFNA_mat = result_PFNA_mat[["results"]],
  PFOA_child = result_PFOA_child[["results"]],
  PFOA_mat = result_PFOA_mat[["results"]],
  PFOS_child = result_PFOS_child[["results"]],
  PFOS_mat = result_PFOS_mat[["results"]],
  PFUnDA_child = result_PFUnDA_child[["results"]],
  PFUnDA_mat = result_PFUnDA_mat[["results"]]
)

save(file="WGCNA_data_for_sierra.RData",
     list=c("WGCNA.dat","covs_PFAS","expr_PFAS","tx2g.assembly",
            "get_genes_by_module_id","cors","sig.cors",
            "pfas_DTE_results","color_ME_mapping"))
