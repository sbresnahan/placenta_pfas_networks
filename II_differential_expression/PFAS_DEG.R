...
get_gene_symbols <- function(ensembl_ids) {
  ...
  df = merge(df,gene_info,by='ensembl_gene_id',all.x=T)
...
run_ruvr_deseq2 <- function(se, pfas_var, covariates = c(), k = 0, output_dir = ".", just_hist = FALSE) {
...
create_volcano_plot <- function(deseq2_results, pfas_var, output_dir = ".", 
  ...
  # Save the plot
  filename <- paste0(output_dir, "/", pfas_var, "_volcanoPlot_DGE.pdf")
...
create_heatmap <- function(deseq2_results, pfas_var, output_dir = ".", 
  ...
  # Save the plot
  ggsave(
    filename = paste0(output_dir, "/", pfas_var, "_significant_genes_heatmap_ggplot_DGE.pdf"),
    plot = final_plot,
    width = 10,
...
csv_out = function(result,pfas_var,output_dir){
  ...
  rrr = result$results
  rrr$ensembl = rownames(result$results)
  ...
  data.table::fwrite(out_rrr,
                     paste0(output_dir, "/", pfas_var, "_DGE.tsv"),
                     sep='\t',
                     col.names=T,
                     row.names=F,
                     quote=F)
...
vis_efcor <- function(result_mat,result_child,exposure,output_dir){
  ...
  df_mat$gene_id <- row.names(df_mat)
  df_child <- as.data.frame(result_child$results)
  df_child$gene_id <- row.names(df_child)
  
  df_merged <- dplyr::inner_join(
    df_mat %>% dplyr::select(gene_id, log2FoldChange, pvalue) %>% dplyr::rename(lfc_maternal = log2FoldChange,
                                                                  pvalue_maternal = pvalue),
    df_child %>% dplyr::select(gene_id, log2FoldChange, pvalue) %>% dplyr::rename(lfc_fetal = log2FoldChange,
                                                                    pvalue_fetal = pvalue),
    by = "gene_id"
  )
  
  # Get concordant genes
  ...
  if (length(concordant) == 0) {
    ...
    concordant <- data.frame(gene_id=concordant)
    ...
    df_merged <- left_join(df_merged,concordant[,c(1,3)])
  ...
  g1 <- ggplot(df_merged, aes(x = lfc_maternal, y = lfc_fetal)) +
    ...
    labs(
      title = paste0(paste(paste0("Correlation of Effect Sizes between","\nMaternal and Fetal"),exposure,sep=" "),
  ...
  ggsave(
    filename = paste0(output_dir, "/", exposure, "_MatChildCorr_DGE.pdf"),
    plot = g3,
    width = 8.8,
    height = 4.22,
    units="in"
...
## Maternal
result <- run_ruvr_deseq2(se = gse_gusto.tx, 
                          pfas_var = all_pfas[1], 
                          covariates = c("sex", "GA"), 
                          k = 4, output_dir = "results", 
                          just_hist = FALSE)
...
result <- run_ruvr_deseq2(se = gse_gusto.tx, 
                          ...
                          just_hist = FALSE)
volcano <- create_volcano_plot(result, pfas_var = all_pfas[2], output_dir = "results")
...
save.image("GUSTO_assembly_DGE.RData")
...
expr <- gse_gusto.tx@assays@data@listData[["counts"]]
...
# --------------------------
# Run SVA with sex in model
# --------------------------
mod1 <- model.matrix(~ sex, data = covs_PFAS)
mod0 <- model.matrix(~ 1, data = covs_PFAS)
sva_result <- sva(dat = expr_PFAS, mod = mod1, mod0 = mod0, n.sv = 1)

# --------------------------
# Regress out SVs only
# --------------------------
expr_corrected <- removeBatchEffect(expr_PFAS,
...
png("WGCA_thresholds_gene.png",width=7,height=5,units="in",res=300)
par(mfrow = c(1,2))
cex1 = 0.5
...
# Select 14 for SFT.R.sq > 0.8 and mean.k ~500

save(file="input_for_WGCNA_gene.RData",list=c("expr_PFAS","covs_PFAS"))

# Continue on seadragon

# Load output from WGCNA analysis on seadragon

load("input_for_WGCNA_gene.RData")
load("WGCNA_output_gene.RData")

# Save output for Sierra

# Create new list with cleaned names and results data frames
pfas_DGE_results <- list(
