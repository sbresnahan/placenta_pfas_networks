...
#
# Data: - Transcript expression quantified using Salmon against long-read 
#         ESPRESSO-corrected placenta-specific transcript annotation
...
a <- load("Elkin2025_Placenta.RData")
...
fig1 <- ggplot(pca.df,aes(x=PC1,y=PC2,color=sample_type)) +
  geom_point() +
  xlab("PC1: 55.29% variance") +
  ylab("PC2: 9.07% variance") +
...
fig2 <- ggplot(pca.df,aes(x=PC1,y=PC2,color=sample_type)) +
  geom_point() +
  xlab("PC1: 27.27% variance") +
  ylab("PC2: 19.06% variance") +
...
#    - PFBS concentrations (0, 5, 20 μM) modeled as continuous scaled variable
#    - Allows detection of linear dose-response relationships
#    - Each unit increase represents 1 standard deviation change in exposure
#
# 2. Biological Relevance:
#    - PFBS concentrations chosen based on environmentally relevant levels
#    - 5 μM ≈ low-moderate exposure scenario
#    - 20 μM ≈ high exposure scenario
#    - Control (0 μM) provides baseline reference
#
# 3. Statistical Approach:
#    - RUVr removes unwanted variation while preserving biological signal
#    - k=8 factors chosen based on exploratory analysis of residual structure
#    - Trimester covariate accounts for developmental stage differences
#    - FDR correction controls false discovery rate at 10%
#
# 4. Transcript vs Gene Level Analysis:
#    - DTE: Identifies specific isoforms responding to PFBS exposure
#    - DGE: Identifies genes showing overall expression changes
#    - ACAT aggregation preserves power while controlling multiple testing
#    - Isoform confirmation testing validates individual transcript effects
#
# 5. Interpretation Guidelines:
#    - Positive log2FC: increased expression with higher PFBS exposure
#    - Negative log2FC: decreased expression with higher PFBS exposure
#    - Effect sizes should be interpreted in context of biological relevance
#    - Focus on genes/transcripts with both statistical significance and meaningful effect sizes
#
# =============================================================================
