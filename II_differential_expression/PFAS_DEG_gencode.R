...
get_gene_symbols <- function(ensembl_ids) {
  ...
  gene_info_dedup <- gene_info %>%
    distinct(ensembl_gene_id, .keep_all = TRUE)   # keep only the first occurrence
  
  # Create a named vector for easy mapping
  df = data.frame(ensembl_gene_id = ensembl_ids)
  df <- left_join(df, gene_info_dedup, by = "ensembl_gene_id")
...
vis_efcor <- function(result_mat,result_child,exposure,output_dir){
  ...
  df_merged <- dplyr::inner_join(
    df_mat %>% dplyr::select(gene_id, log2FoldChange, pvalue) %>% rename(lfc_maternal = log2FoldChange,
                                                                  pvalue_maternal = pvalue),
    df_child %>% dplyr::select(gene_id, log2FoldChange, pvalue) %>% rename(lfc_fetal = log2FoldChange,
                                                                    pvalue_fetal = pvalue),
    by = "gene_id"
  )
...
result <- run_ruvr_deseq2(se = gse_gusto.tx, 
                          ...
                          just_hist = F)
volcano <- create_volcano_plot(result$results, pfas_var = all_pfas[6], 
                               output_dir = "results_gencode", k=2)
...
result <- run_ruvr_deseq2(se = gse_gusto.tx, 
                          ...
                          k = 2, output_dir = "results_gencode", 
                          just_hist = FALSE)
volcano <- create_volcano_plot(result$results, pfas_var = all_pfas[14], 
                               output_dir = "results_gencode", k=2)
