...
# saveRDS(expr_PFAS, "expr_PFAS.rds")
# saveRDS(covs_PFAS, "covs_PFAS.rds")
...
counts$category <- gsub("DTE", "DET", counts$category)
counts$label <- gsub("DTE", "DET", counts$label)

mediation_DTE_vs_Hub_ACME <- data.frame(mediation_DTE_vs_Hub_ACME)
mediation_DTE_vs_Hub_ACME$category <- gsub("DTE", "DET", mediation_DTE_vs_Hub_ACME$category)
mediation_DTE_vs_Hub_ACME$category <- factor(mediation_DTE_vs_Hub_ACME$category, 
                                             levels = c("DET", "Hub (Non-mediator)", "Hub (Mediator)"))
...
ggsave("dte_vs_hub_ACME.pdf", g1, width = 6.75, height = 6, units = "in", dpi = 300)
...
p_ade_forest <- ggplot(ade_effects, aes(x = PFAS, y = ADE, color = exposure_label)) +
  ...
  labs(
    ...
    title = "Direct effects of PFAS on birth weight\nvary by exposure source and compound"
  ) +
...
saveRDS(ade_effects,"Figures/ade_effects.RDS")

ggsave("p_ade_forest.pdf", p_ade_forest, width = 6.75, height = 6, units = "in", dpi = 300)
...
p_kde_enhanced <- plot_histogram_data %>%
  ...
  labs(
    ...
    y = "Density",
    title = "Co-expression strength of PFAS-birth weight mediator\ntranscripts scales with transplacental transfer efficiency (TPTE)",
...
ggsave("p_kde_all_pfas.pdf", p_kde_enhanced, width = 5.25, height = 4.5, units = "in", dpi = 300)
...
p_tpte_vs_peak <- peak_data_summary %>%
  ...
  labs(
    ...
    title = "Co-expression strength of fetal PFAS-birth weight mediator\ntranscripts scales with transplacental transfer efficiency",
...
ggsave("p_tpte_vs_peak_kme.pdf", p_tpte_vs_peak, width = 6.75, height = 6, units = "in", dpi = 300)


save(list=c("tpte_stats_for_plot","peak_data_summary"),file="Figures/fig3c_tx.RData")

BW.COEX <- peak_data_summary %>%
  group_by(exposure_label) %>%
  do({
...
BW.COEX.SLOPES <- peak_data_summary %>%
  group_by(exposure_label) %>%
  do({
    mod <- lm(peak_x_mean ~ mean_tpte, data = .)
  ...
  ungroup()
BW.COEX.SLOPES$outcome <- "Birth weight"
BW.COEX.SLOPES$metric <- "Co-expression strength"

saveRDS(BW.COEX,"bw_coex.RDS")
saveRDS(BW.COEX.SLOPES,"bw_coex_slopes.RDS")
...
p_acme_boxplot <- ggplot(acme_tpte_sig, aes(x = PFAS, y = abs_ACME, color = exposure_label)) +
  ...
  labs(
    ...
    title = "PFAS-birth weight mediation effects vary by exposure source and compound"
  ) +
  theme_classic(base_size = 12) +
  theme(
    plot.title = element_text(face = "bold"),
...
ggsave("p_tpte_vs_mean_acme.pdf", p_acme_boxplot, width = 7.25, height = 7, units = "in", dpi = 300)
...
p_acme_ridges <- ggplot(acme_tpte_sig, aes(x = ACME, y = PFAS, fill = exposure_label)) +
  ...
  labs(
    ...
    y = "PFAS",
    title = "PFAS-birth weight effects mediated by placental transcript\nexpression vary by exposure source and compound"
  ) + theme_classic(base_size = 12) +
  theme(
    plot.title = element_text(face = "bold"),
    legend.position = "top")
...
ggsave("p_tpte_vs_mean_acme_ridges.pdf", p_acme_ridges, width = 6.75, height = 6,, units = "in", dpi = 300)
...
label_y_spacing <- max(mediator_counts$n_significant) * 0.1
...
ggsave("mediator_scatterplot.pdf", p, width = 6.75, height = 6, units="in", dpi = 300)

save(list=c("cor_stats","mediator_counts"),file="Figures/fig3b_tx.RData")



BW.MC <- mediator_counts %>%
  group_by(exposure_label) %>%
  do({
...
BW.MC.SLOPES <- mediator_counts %>%
  ...
  ungroup()
BW.MC.SLOPES$outcome <- "Birth weight"
BW.MC.SLOPES$metric <- "Mediator count"

saveRDS(BW.MC,"bw_mc.RDS")
saveRDS(BW.MC.SLOPES,"bw_mc_slopes.RDS")
...
p_module_boxplot <- module_enrichment_with_fisher %>%
  ...
  labs(
    ...
    title = "Co-expression network enrichment for PFAS-birth weight mediators\nvaries by exposure source and compound",
...
ggsave("p_module_boxplot.pdf", p_module_boxplot, width = 7.25, height = 6.75, units="in", dpi = 300)
...
label_x_start <- 0.2
label_y_start <- 0.06
label_y_spacing <- label_y_start * 0.25
...
ggsave("p_avg_prop_sig_vs_peak_kme.pdf", p_avg_prop_vs_peak_kme, 
