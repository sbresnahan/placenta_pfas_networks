...
result_files <- list.files("WGCNA_mediation_results_gene", pattern = "mediation_batch_.*\\.rds", full.names = TRUE)
...
save.image("acme_vs_kME_gene.RData")
...
# Get all RData files
rdata_files <- list.files(bootstrap_dir, pattern = "^WGCNA_output_\\d+\\_gene.RData$", full.names = TRUE)
...
########## Compare ACME between DGEs and hubs
...
for (pfas in pfas_compounds) {
  for (exposure in c("mat", "child")) {
    dte_file <- file.path(results_dir, paste0(pfas, "_", exposure, "_DGE.tsv"))
    if (!file.exists(dte_file)) next
    
    dte_data <- read_tsv(dte_file, show_col_types = FALSE)
    names(dte_data)[1] <- "Transcript"
...
levels(mediation_DTE_vs_Hub_ACME$category)[levels(mediation_DTE_vs_Hub_ACME$category) == "DTE"] <- "DEG"

counts$category <- factor(counts$category, 
                          levels = c("DTE", "Hub (Non-mediator)", "Hub (Mediator)"),
                          labels = c("DEG", "Hub (Non-mediator)", "Hub (Mediator)"))

counts$label <- gsub("DTE", "DEG", counts$label)
...
ggsave("dte_vs_hub_ACME_gene.pdf", g1, width = 6.75, height = 6, units = "in", dpi = 300)
...
p_acme_ridges <- ggplot(acme_tpte_sig, aes(x = ACME, y = PFAS, fill = exposure_label)) +
  ...
  labs(
    ...
    y = "PFAS",
    title = "PFAS-birth weight effects mediated by placental gene\nexpression vary by exposure source and compound"
...
ggsave("p_tpte_vs_mean_acme_ridges_gene.pdf", p_acme_ridges, width = 6.75, height = 6,, units = "in", dpi = 300)
...
ade_effects <- all_module_mediation_df %>%
  ...
  summarise(
    ADE = mean(ADE, na.rm = TRUE),
    ADE_SE = sqrt(sum(ADE_SE^2)) / n(),
    n = n(),
    .groups = "drop"
  ) %>%
  mutate(
    ADE_CI_lower = ADE - ADE_SE,
    ADE_CI_upper = ADE + ADE_SE
  ) %>%
...
p_ade_forest <- ggplot(ade_effects, aes(x = PFAS, y = ADE, color = exposure_label)) +
  ...
  labs(
    ...
    title = "Effects of PFAS on birth weight vary by exposure and compound"
  ) +
  theme_classic(base_size = 12) +
  theme(
    plot.title = element_text(face = "bold"),
...
p_ade_forest # Fix the summarization here

ggsave("p_ade_forest_gene.pdf", p_ade_forest, width = 6.75, height = 6, units = "in", dpi = 300)
...
p_kde_enhanced <- plot_histogram_data %>%
  ...
  labs(
    ...
    y = "Density",
    title = "Co-expression strength of birth weight mediator genes",
...
ggsave("p_kde_all_pfas_gene.pdf", p_kde_enhanced, width = 5.25, height = 4.5, units = "in", dpi = 300)
...
p_tpte_vs_peak <- peak_data_summary %>%
  ...
  labs(
    ...
    title = "Co-expression strength of PFAS-birth weight mediator\ngenes does not scale with transplacental transfer efficiency",
...
ggsave("p_tpte_vs_peak_kme_gene.pdf", p_tpte_vs_peak, width = 6.75, height = 6, units = "in", dpi = 300)

save(list=c("tpte_stats_for_plot","peak_data_summary"),file="Figures/fig3c_gene.RData")
...
ggsave("mediator_scatterplot_gene.pdf", p, width = 6.75, height = 6, units="in", dpi = 300)

save(list=c("cor_stats","mediator_counts"),file="Figures/fig3b_gene.RData")
...
ggsave("p_module_boxplot_gene.pdf", p_module_boxplot, width = 7.25, height = 6.75, units="in", dpi = 300)
...
label_x_start <- 0.4
label_y_start <- 0.04
label_y_spacing <- label_y_start * 0.2
...
p_avg_prop_vs_peak_kme <- plot_data_summary %>%
  ...
  labs(
    ...
    title = "Average module enrichment for PFAS-birth weight\nmediator transcripts does not scale\nwith peak module membership"
  ) +
  theme_classic(base_size = 12) +
  theme(
    plot.title = element_text(face = "bold"),
...
ggsave("p_avg_prop_sig_vs_peak_kme_gene.pdf", p_avg_prop_vs_peak_kme, 
