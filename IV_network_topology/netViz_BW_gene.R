...
load("~/Desktop/PFAS/acme_vs_kME_gene.RData")

expr_PFAS <- readRDS("expr_PFAS_gene.RDS")
adjacency <- readRDS("adjacency_gene.RDS")
...
ggsave("p_kde_distance_mediators_gene.pdf", p_kde_distance, width = 12, height = 8, units = "in", dpi = 300)
...
get_density_peak <- function(x) {
  x <- x[is.finite(x)]  # Remove NA, NaN, Inf, -Inf
...
ggsave("p_tpte_vs_peak_distance_gene.pdf", p_tpte_vs_peak_distance, width = 6.75, height = 6, units = "in", dpi = 300)

save(list=c("tpte_stats_distance","peak_distance_summary"),file="Figures/fig4b_gene.RData")
...
p_peak_kme_vs_peak_distance <- ggplot(
  ...
  labs(
    x = "Peak |kME|",
    y = "Peak distance from central hub",
    title = "Network centrality of maternal PFAS-birth weight\nmediator genes scales with module membership",
    subtitle = "Points show mean ± SE across iterations"
  ) +
  theme_classic(base_size = 12) +
  theme(
...
ggsave("p_peak_kme_vs_peak_distance_gene.pdf", p_peak_kme_vs_peak_distance, 
...
gdtpte <- ggplot(pfas_peaks, aes(x = mean_tpte, y = peak_abs_diff)) +
  ...
  labs(
    ...
    title = "Distance between mediator hub genes does not scale with TPTE"
  )

gdtpte

ggsave("gdtpte_gene.pdf", gdtpte, 
       width = 6.75, height = 6, units = "in", dpi = 300)

save(list=c("cor_label","pfas_peaks"),file="Figures/fig4c_gene.RData")
...
ggsave("p_acme_vs_peak_distance_avg_gene.pdf", p_acme_vs_peak_distance,
       width = 6.75, height = 6, units = "in", dpi = 300)
